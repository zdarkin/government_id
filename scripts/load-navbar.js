...
function adjustNavbarLinks(isInPagesFolder) {
  ...
  const linkMap = Object.fromEntries(
    navHrefs.map((href) => [href, resolvePath(href, isInPagesFolder)]),
  );
  ...
  document.querySelectorAll("#navLinks a").forEach((link) => {
    ...
    if (
      originalHref === currentPage ||
      (currentPage === "" && originalHref === "index.html")
    ) {
  ...
  if (logoImg) {
    logoImg.setAttribute(
      "src",
      isInPagesFolder ? "../assets/images/logo.png" : "assets/images/logo.png",
    );
  }
  ...
  if (logoLink) {
    logoLink.setAttribute(
      "href",
      isInPagesFolder ? "../index.html" : "index.html",
    );
  }
}
...
function adjustFooterLinks(isInPagesFolder) {
  const footerHrefs = [
    ...
    "about.html",
    "contact.html",
    "category.html?cat=national-civil",
    "category.html?cat=employment",
    "category.html?cat=licensure",
    "category.html?cat=special-groups",
    "category.html?cat=clearances",
    "category.html?cat=institutional",
  ];
  const linkMap = Object.fromEntries(
    footerHrefs.map((href) => [href, resolvePath(href, isInPagesFolder)]),
  );
...
function initHamburgerMenu() {
  ...
  if (hamburger && navLinks) {
    ...
    // Close menu when a link is clicked (optimized using event delegation)
...
function initGlobalSearch(isInPagesFolder) {
  ...
  scriptDetails.src = isInPagesFolder
    ? "../scripts/data-details.js"
    : "scripts/data-details.js";

  scriptDetails.onload = () =>
    attachSearchListener(searchInput, suggestionsBox, isInPagesFolder);
  ...
  searchInput.addEventListener("input", () => {
    ...
    for (const catKey in idDatabase) {
      ...
      cat.items.forEach((item) => {
        ...
        const reqsStr = (detailedInfo.requirements || [])
          .join(" ")
          .toLowerCase();
